...
/* 历史缓冲区定义（注释保留原始设计意图） */
//unsigned char  mqtt_TxBuf[7][400];  // 原始发送缓冲矩阵（7通道*400字节，已停用）
//unsigned char *mqtt_TxInPtr;        // 发送缓冲区写入指针
//unsigned char *mqtt_TxOutPtr;       // 发送缓冲区读取指针
//unsigned char *mqtt_TxEndPtr;       // 发送缓冲区结束地址指针
...
/* 历史指令缓冲区定义（注释保留原始设计） */
//unsigned char  mqtt_CMDBuf[7][400]; // 指令缓冲矩阵（已停用）
//unsigned char *mqtt_CMDInPtr;       // 指令写入指针
//unsigned char *mqtt_CMDOutPtr;      // 指令读取指针
//unsigned char *mqtt_CMDEndPtr;      // 指令缓冲区结束指针
 ...
 *   2. 根据长度值循环发送后续字节数据
 * 设计说明：
 *   - 保留被注释的原始代码（send_data_to_dev(data, strlen(data))）体现协议改进过程
 *   - 当前实现规避strlen风险（数据可能含'\0'或非字符串内容）
 *-------------------------------------------*/
 ...
 *   QoS - 服务质量等级（0/1/2）
 * 注：保留原始计算逻辑，未做边界检查（需确保topic_name长度合法）
 *-----------------------------------------------------------*/
...
{
    /* 历史环形缓冲区方案（已注释弃用） */
    //memcpy(&mqtt_CMDInPtr[2], data, size);  // 数据拷贝（偏移2字节）
    //mqtt_CMDInPtr[0] = size / 256;         // 长度高位存储
    //mqtt_CMDInPtr[1] = size % 256;         // 长度低位存储
    //mqtt_CMDInPtr[size + 2] = '\0';        // 添加终止符（非标准MQTT协议）
    //mqtt_CMDInPtr += 400;                  // 环形缓冲区指针移动
    //if (mqtt_CMDInPtr >= mqtt_CMDEndPtr) 
    //{
    //    mqtt_CMDInPtr = mqtt_CMDBuf[0];    // 缓冲区回绕
    //}
 ...
 *   4. 原始注释"有必要吗？很有必要"强调终止符必要性（需调用方保证dst空间>=n+1）
 *-----------------------------------------------------------*/
...
//    }
    ...
    if (is_new_CMD_rx == 1)//(mqtt_CMDOutPtr != mqtt_CMDInPtr) 
    {
        ...
        if (strstr((char *)CMD_rx_buff + 2, "Automatic")) {
			oled_Clear();
			mode_selection = 0;
		}
		if (strstr((char *)CMD_rx_buff + 2, "Manual")) {
			oled_Clear();
			mode_selection = 2;	 
		System.Switch1 = System.Switch2 = System.Switch3 = System.Switch4 = 0;
		}
		
		if (strstr((char *)CMD_rx_buff + 2, "ThresholdON")) {
			oled_Clear();
			mode_selection = 1;
		}
		if (strstr((char *)CMD_rx_buff + 2, "ThresholdOFF")) {
			oled_Clear();
			mode_selection = 0;
		}

		
		if (strstr((char *)CMD_rx_buff + 2, "Switch1ON")) {
			if(manualFlag >=3){
				oled_Clear();
			}
			manualFlag = 0;
		...
		if (strstr((char *)CMD_rx_buff + 2, "Switch1OFF")) {
			if(manualFlag >=3){
				oled_Clear();
			}
			manualFlag = 0;
		...
		if (strstr((char *)CMD_rx_buff + 2, "Switch2ON")) {
			if(manualFlag >=3){
				oled_Clear();
			}
			manualFlag = 1;
		...
		if (strstr((char *)CMD_rx_buff + 2, "Switch2OFF")) {
			if(manualFlag >=3){
				oled_Clear();
			}
			manualFlag = 1;
		...
		if (strstr((char *)CMD_rx_buff + 2, "Switch3ON")) {
			if(manualFlag >=3){
				oled_Clear();
			}
			manualFlag = 2;
		...
		if (strstr((char *)CMD_rx_buff + 2, "Switch3OFF")) {
			if(manualFlag >=3){
				oled_Clear();
			}
			manualFlag = 2;
		...
		if (strstr((char *)CMD_rx_buff + 2, "Switch4ON")) {
			if(manualFlag <3){
				oled_Clear();
			}
			manualFlag = 3;
		...
		if (strstr((char *)CMD_rx_buff + 2, "Switch4OFF")) {
			if(manualFlag <3){
				oled_Clear();
			}
			manualFlag = 3;
		...
		if (strstr((char *)CMD_rx_buff + 2, "TempThresholdAdd")) {
			if(thresholdFlag >=3){
				oled_Clear();
			}
			mode_selection = 1;
		...
		if (strstr((char *)CMD_rx_buff + 2, "TempThresholdDown")) {
			if(thresholdFlag >=3){
				oled_Clear();
			}
			mode_selection = 1;
		...
		if (strstr((char *)CMD_rx_buff + 2, "SoilThresholdAdd")) {
			if(thresholdFlag >=3){
				oled_Clear();
			}
			mode_selection = 1;
		...
		if (strstr((char *)CMD_rx_buff + 2, "SoilThresholdDown")) {
			if(thresholdFlag >=3){
				oled_Clear();
			}
			mode_selection = 1;
		...
		if (strstr((char *)CMD_rx_buff + 2, "LightThresholdAdd")) {
			if(thresholdFlag >=3){
				oled_Clear();
			}
			mode_selection = 1;
			thresholdFlag = 2;
			Threshold.LightThreshold +=10;
		}
		if (strstr((char *)CMD_rx_buff + 2, "LightThresholdDown")) {
			if(thresholdFlag >=3){
				oled_Clear();
			}
			mode_selection = 1;
